...
func (tree *ObjectTree) Load() (maxOid uint64, err error) {
	f := tree.idxFile
	maxOid, err = WalkIndexFile(f, func(oid, offset uint64, size, crc uint32) error {
...
func WalkIndexFile(f *os.File, fn func(oid, offset uint64, size, crc uint32) error) (maxOid uint64, err error) {
	var (
		readOff int64
		count   int
		iter    int
